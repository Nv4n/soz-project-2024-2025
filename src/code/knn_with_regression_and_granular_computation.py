...
print(
    f"Head records of Books:\n{all_books.head()}\nHead records of users:\n{all_users.head()}\nHead records of ratings:\n{all_ratings.head()}\n"
)

merged_data = pd.merge(
    all_ratings.dropna(), all_users[["User-ID", "Age"]], on="User-ID", how="inner"
)
print(f"Merged data:\n{merged_data}\n")
...
print(
    f"P-value: {p_value}"
)  # equals to zero => no linear relationship between readers' age and books' rating.
...
def recommend_books(
    user_data, k=5, weights=np.array([2, 1])
):  # address that the book's rating will be taken with
    ...
    for book_id in book_ids:
        book_info = all_books[all_books["ISBN"] == book_id].iloc[
            0
